...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`supports-color`](https://crates.io/crates/supports-color).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `supports-color` crate will be cloned into `tmp/` (gitignored) when porting begins.

## About

Probe terminal for ANSI-color support
...
Original work copyrighted by the upstream `supports-color` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
