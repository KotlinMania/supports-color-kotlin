# Claude Code Project Instructions — supports-color-kotlin

## Project Overview

This is **supports-color-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`supports-color`](https://crates.io/crates/supports-color).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/supports-color/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/supports-color>
